...
class Functions {
	...
	public function mostra_utente($id_utente) {
		$query = @mysql_query("SELECT username FROM user WHERE username = $id_utente") or die('Errore: ' . mysql_error());
	...
	public function verifica_login($email_o_nome_utente, $password) {
		...
		$query = mysqli_query($link, "SELECT email FROM user WHERE (email = '$email_o_nome_utente' OR username='$email_o_nome_utente') AND password = '$password2'") or die('Errore: ' . mysqli_error($link));
